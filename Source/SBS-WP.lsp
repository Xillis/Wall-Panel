...
;;Panel insert loop
(defun SBS-CALC-LOOP (TOP BOTTOM PINFO PW / COUNT PINFO WLENGTH PW RUNLENGTH BPOINTS TPOINTS FLAG)
	...
	(WHILE ( < RUNLENGTH WLENGTH)
		...
		(SBS-PANELINSERT 
			(COND
				((AND (> (CADR (CADR BPOINTS)) (CADR (CAR BPOINTS))) (NOT (MINUSP PW)) (NOT (= (CAR (CAR BPOINTS)) (CAR (CADR BPOINTS)))))
					(LIST
						(CAR (CAR BPOINTS))
						(+ (CADR (CAR BPOINTS)) (SETQ FLAG (*(/ (- (CADR (CADR BPOINTS)) (CADR (CAR BPOINTS))) (- (CAR (CADR BPOINTS)) (CAR (CAR BPOINTS)))) PW))) 
						(CADDR (CAR BPOINTS))
					)
				)
				((AND (< (CADR (CADR BPOINTS)) (CADR (CAR BPOINTS))) (MINUSP PW) (NOT (= (CAR (CAR BPOINTS)) (CAR (CADR BPOINTS)))))
					(LIST
						(CAR (CAR BPOINTS))
						(- (CADR (CAR BPOINTS)) (SETQ FLAG (*(/ (- (CADR (CADR BPOINTS)) (CADR (CAR BPOINTS))) (- (CAR (CADR BPOINTS)) (CAR (CAR BPOINTS)))) PW))) 
						(CADDR (CAR BPOINTS))
			...
			(COND
				((AND (< (CADR (CADR TPOINTS)) (CADR (CAR TPOINTS))) (NOT (MINUSP PW)) (NOT (= (CAR (CAR TPOINTS)) (CAR (CADR TPOINTS)))))
					(- (+ (- (CADR (CAR TPOINTS)) (CADR (CAR BPOINTS))) (*(/ (- (CADR (CADR TPOINTS)) (CADR (CAR TPOINTS))) (- (CAR (CADR TPOINTS)) (CAR (CAR TPOINTS)))) PW)) FLAG)
				)
				((AND (> (CADR (CADR TPOINTS)) (CADR (CAR TPOINTS))) (MINUSP PW) (NOT (= (CAR (CAR TPOINTS)) (CAR (CADR TPOINTS)))))
					(+ (- (- (CADR (CAR TPOINTS)) (CADR (CAR BPOINTS))) (*(/ (- (CADR (CADR TPOINTS)) (CADR (CAR TPOINTS))) (- (CAR (CADR TPOINTS)) (CAR (CAR TPOINTS)))) PW)) FLAG)
				)
				(T (- (- (CADR (CAR TPOINTS)) (CADR (CAR BPOINTS)))flag))
			)
		PINFO)
		(PRINT "FLAG")
		(PRINT FLAG)
