...
;;Panel insert loop
(defun SBS-CALC-LOOP (TOP BOTTOM PINFO PW / COUNT PINFO WLENGTH PW RUNLENGTH BPOINTS TPOINTS)
	...
	(WHILE ( < RUNLENGTH WLENGTH)
		...
		(SBS-PANELINSERT 
			(CAR BPOINTS)
			(COND
				((AND (< (CADR (CADR TPOINTS)) (CADR (CAR TPOINTS))) (NOT (MINUSP PW)))
					(+ (- (CADR (CAR TPOINTS)) (CADR (CAR BPOINTS))) (*(/ (- (CADR (CADR TPOINTS)) (CADR (CAR TPOINTS))) (- (CAR (CADR TPOINTS)) (CAR (CAR TPOINTS)))) PW))
				)
				((AND (> (CADR (CADR TPOINTS)) (CADR (CAR TPOINTS))) (MINUSP PW))
					(- (- (CADR (CAR TPOINTS)) (CADR (CAR BPOINTS))) (*(/ (- (CADR (CADR TPOINTS)) (CADR (CAR TPOINTS))) (- (CAR (CADR TPOINTS)) (CAR (CAR TPOINTS)))) PW))
				)
				(T (- (CADR (CAR TPOINTS)) (CADR (CAR BPOINTS))))
			)
		PINFO)
		;;Base Points
