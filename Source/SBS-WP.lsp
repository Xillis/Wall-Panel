...
(defun SBS-WALL-PANEL-CALC ( PASSTHROUGH / PASSTHROUGH SPOINTS top bottom RUNLENGTH WLENGTH PINFO oldvar BPOINTS TPOINTS)
	...
	(WHILE ( < RUNLENGTH WLENGTH)
		...
		(SBS-PANELINSERT (CAR BPOINTS) (- (CADR (CAR TPOINTS)) (CADR (CAR BPOINTS))) PINFO)
		;;Base Points
		;;(VL-CMDF "._CIRCLE" (CAR BPOINTS) "_D" (ATOI (CADR (CADR PINFO))))
		(SETQ BPOINTS (SBS-POINT-CALC BPOINTS (ATOI (CADR (CADR PINFO))) BOTTOM ))
		;;Top points
		;;(VL-CMDF "._CIRCLE" (CAR TPOINTS) "_D" (ATOI (CADR (CADR PINFO))))
		(SETQ TPOINTS (SBS-POINT-CALC TPOINTS (ATOI (CADR (CADR PINFO))) TOP ))
	)
...
(DEFUN SBS-POINT-CALC (POINTS WIDTH PLIST / POINTS P1 P2 P3 WIDTH PLIST)
	...
	(COND
		;;RUNNING CONDITION
		((> (DISTANCE (LIST (CAR P1) 0.0 0.0) (LIST (CAR P2) 0.0 0.0)) WIDTH)
			(SETQ P1
				(LIST 
					(+ (CAR P1) WIDTH)
		...
		((< (- (CAR (CADR (MEMBER P2 PLIST))) (CAR P1)) WIDTH)
			(SETQ P3 (CADR (MEMBER P2 PLIST)))
			(WHILE (< (- (CAR (CADR (MEMBER P3 PLIST))) (CAR P1)) WIDTH)
